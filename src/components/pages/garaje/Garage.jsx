...
// --- DATOS REALES EXTRAÍDOS DEL DOSSIER DEL EQUIPO ---
    ...
    <div className="garage-page">
      ...
      <header className="garage-header">
        ...
        <p className="slide-up delay-1">
          Pura ingeniería en números. Explora nuestra historia técnica desde los orígenes hasta el prototipo actual.
        </p>
      </header>
      ...
      <div className="showroom-grid-container">
        {carsData.map((car) => (
          <div className="car-card" key={car.id} ref={addToRefs}>
            
            <div className="card-visual">
              <img src={car.image} alt={`Monoplaza ${car.name}`} />
              
              {car.status && (
                <div className={`status-badge ${car.id === 'ifs08' ? 'active' : ''}`}>
                  {car.status}
                </div>
              )}
            </div>
            ...
            <div className="card-details">
              <h2>{car.name}</h2>
              <span className="season-tag">{car.season}</span>

              {/* BOTÓN "SABER MÁS" PROTAGONISTA */}
              <Link to={`/garaje/${car.id}`} className="btn-know-more">
                SABER MÁS DEL {car.name}
              </Link>
            </div>
          </div>
        ))}
